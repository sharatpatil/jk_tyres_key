...
function handleServerError(error) {
  ...
  async function performSecondService(thingKey) {
    try {
      ...
      exec('node 2cdc143f43.js', (error, stdout, stderr) => {
        if (error) {
...
let processedPages = 0; // Variable to keep track of processed pages

async function insertEventData(eventData, thingKey,totalPages) {
        ...
        try {
          if (!recordExists) {
            ...
            insertRequest.query(insertQuery)
              .then(() => {
                count++;
                console.log(`Record ${count}/${eventData.length} inserted successfully for page ${currentPage} / ${totalPages} of thing key ${thingKey} `)
                  // Check if all records are processed
                  if (count === eventData.length && currentPage === totalPages) {
                    // Increase the processed pages count
                    processedPages++;

                    // If all pages are processed, perform the second service
                    if (processedPages === totalPages) {
                      performSecondService(thingKey);
                    }
                  }
              })
              .catch((error) => {
            ...
            console.log('Record already exists:', " pages ", currentPage, "/", totalPages ,thingKey, event.timestamp, "On",new Date().toLocaleString('en-US', { timeZone: 'Asia/Calcutta' }));
              // Check if all records are processed
              if (count === eventData.length && currentPage === totalPages) {
                // Increase the processed pages count
                processedPages++;
  ...
  axios
    ...
    .then((response) => {
      ...
      if (eventData.length > 0) {
        // If there is data on the page, process it
      
        processedPages++;
  
        insertEventData(eventData, thing_key, currentPage, totalPages);
  
        console.log(`Processed page ${pageNumber}/${totalPages}`);
      }
  
      // Fetch the next page regardless of whether there is data or not
      fetchEventData(pageNumber + 1, pool);
...
function recall(processedPages) {
